...
void SJF() 
{
    ...
    for (i = 0; i < p_num - 1; i++)
    {
        ...
        for (j = i + 1; j < p_num; j++)
        {
            if ((R[j] < R[idx]) || ((R[j] == R[idx]) && (T[j] < T[idx])))
            {
                idx = j;
            }
        }
 ...
 //       printf("%s\t%d\t%d\n", N[i], R[i], T[i]);
    ...
    //    printf("----------------\n");
        finish_time += T[P[i]];
     //   printf("Finish time is %d after P[%d] is done.\n",finish_time, P[i]);
       // printf("Finish time is %d after P[%d] is done.\n",finish_time, P[i]);
        for (j = i; j < p_num; j++) 
        {
        ...
        for (j = point - 1; j >= i + 1; j--)
        {
            for (k = i + 1; k < j; k++)
            {
                if (T[P[k]] > T[P[k + 1]])
                {
                    swap_child(&P[k], &P[k + 1]);
                }
            }
        }
